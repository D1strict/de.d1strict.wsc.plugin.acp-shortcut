{if $__wcf->session->getPermission('admin.general.canUseACPShortcut') && $__wcf->session->getPermission('admin.general.canUseAcp')}
  ...
  <li id="d1ACPShortcut" class="d1ACPShortcut">
    <a href="{link isACP=true}{/link}"{if NEWTAB_ACPSHORTCUT} target="_blank"{/if} class="jsTooltip" title="{lang}wcf.global.acp.short{/lang}" data-tooltip="{lang}wcf.global.acp.short{/lang}" aria-label="{lang}wcf.global.acp.short{/lang}" role="button" tabindex="0" aria-haspopup="true" aria-expanded="false">
